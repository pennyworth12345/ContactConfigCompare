class CfgEditorSubcategories
{
	class EdSubcat_AAs
	{
		displayName = "Anti-Air";
	};
	class EdSubcat_Advertisements
	{
		displayName = "Advertisements";
	};
	class EdSubcat_Agricultural
	{
		displayName = "Agricultural";
	};
	class EdSubcat_AircraftCarrier
	{
		displayName = "Aircraft Carrier [Static]";
	};
	class EdSubcat_Airports
	{
		displayName = "Airport";
	};
	class EdSubcat_APCs
	{
		displayName = "APCs";
	};
	class EdSubcat_Aquatic
	{
		displayName = "Aquatic";
	};
	class EdSubcat_Artillery
	{
		displayName = "Artillery";
	};
	class EdSubcat_AssaultRifles
	{
		displayName = "Assault Rifles";
	};
	class EdSubcat_Avian
	{
		displayName = "Avian";
	};
	class EdSubcat_Backpacks
	{
		displayName = "Backpacks";
	};
	class EdSubcat_Beach
	{
		displayName = "Beach";
	};
	class EdSubcat_BlankSigns
	{
		displayName = "Blanks";
	};
	class EdSubcat_Blocks
	{
		displayName = "Blocks";
	};
	class EdSubcat_Boats
	{
		displayName = "Boats";
	};
	class EdSubcat_BottomSlot
	{
		displayName = "Bipods";
	};
	class EdSubcat_Bushes
	{
		displayName = "Bushes";
	};
	class EdSubcat_Camping
	{
		displayName = "Camping";
	};
	class EdSubcat_Cars
	{
		displayName = "Cars";
	};
	class EdSubcat_Cemetery
	{
		displayName = "Cemetery";
	};
	class EdSubcat_Chemlights
	{
		displayName = "Chemlights";
	};
	class EdSubcat_Clutter
	{
		displayName = "Clutter";
	};
	class EdSubcat_ConstructionSites
	{
		displayName = "Construction Sites";
	};
	class EdSubcat_Craters
	{
		displayName = "Craters";
	};
	class EdSubcat_Decals
	{
		displayName = "Decals";
	};
	class EdSubcat_Decontamination
	{
		displayName = "Decontamination";
	};
	class EdSubcat_Default
	{
		displayName = "Other";
	};
	class EdSubcat_DismantledWeapons
	{
		displayName = "Dismantled Weapons";
	};
	class EdSubcat_Drones
	{
		displayName = "Drones";
	};
	class EdSubcat_Electronics
	{
		displayName = "Electronics";
	};
	class EdSubcat_Explosives
	{
		displayName = "Explosives";
	};
	class EdSubcat_Exterrestrial
	{
		displayName = "Extraterrestrial";
	};
	class EdSubcat_Fire
	{
		displayName = "Fire";
	};
	class EdSubcat_Flags
	{
		displayName = "Flags";
	};
	class EdSubcat_Food
	{
		displayName = "Food";
	};
	class EdSubcat_Forest
	{
		displayName = "Forest";
	};
	class EdSubcat_FrontSlot
	{
		displayName = "Suppressors";
	};
	class EdSubcat_Garbage
	{
		displayName = "Junk";
	};
	class EdSubcat_Graffiti
	{
		displayName = "Graffiti";
	};
	class EdSubcat_Hats
	{
		displayName = "Hats";
	};
	class EdSubcat_Helicopters
	{
		displayName = "Helicopters";
	};
	class EdSubcat_Helipads
	{
		displayName = "Helipads";
	};
	class EdSubcat_Helmets
	{
		displayName = "Helmets";
	};
	class EdSubcat_Helpers
	{
		displayName = "Helpers";
	};
	class EdSubcat_Historical
	{
		displayName = "Historical";
	};
	class EdSubcat_Humanitarian
	{
		displayName = "Humanitarian";
	};
	class EdSubcat_Industrial
	{
		displayName = "Industrial";
	};
	class EdSubcat_Intel
	{
		displayName = "Intel";
	};
	class EdSubcat_InventoryItems
	{
		displayName = "Inventory Items";
	};
	class EdSubcat_Lamps
	{
		displayName = "Lamps";
	};
	class EdSubcat_Launchers
	{
		displayName = "Launchers";
	};
	class EdSubcat_MachineGuns
	{
		displayName = "Machine Guns";
	};
	class EdSubcat_Machines
	{
		displayName = "Machines";
	};
	class EdSubcat_Magazines
	{
		displayName = "Magazines";
	};
	class EdSubcat_Market
	{
		displayName = "Market";
	};
	class EdSubcat_Medicine
	{
		displayName = "Medicine";
	};
	class EdSubcat_Military
	{
		displayName = "Military";
	};
	class EdSubcat_Obstacles
	{
		displayName = "Obstacles";
	};
	class EdSubcat_Office
	{
		displayName = "Office";
	};
	class EdSubcat_Personnel
	{
		displayName = "Men";
	};
	class EdSubcat_Personnel_African
	{
		displayName = "Men (Africans)";
	};
	class EdSubcat_Personnel_Asian
	{
		displayName = "Men (Asian)";
	};
	class EdSubcat_Personnel_Bandits
	{
		displayName = "Men (Bandits)";
	};
	class EdSubcat_Personnel_Camo_Arctic
	{
		displayName = "Men (Arctic)";
	};
	class EdSubcat_Personnel_Camo_Arid
	{
		displayName = "Men (Arid)";
	};
	class EdSubcat_Personnel_Camo_Desert
	{
		displayName = "Men (Desert)";
	};
	class EdSubcat_Personnel_Camo_Jungle
	{
		displayName = "Men (Jungle)";
	};
	class EdSubcat_Personnel_Camo_Urban
	{
		displayName = "Men (Urban)";
	};
	class EdSubcat_Personnel_Camo_Woodland
	{
		displayName = "Men (Woodland)";
	};
	class EdSubcat_Personnel_CombatPatrol
	{
		displayName = "Men (Combat Patrol)";
	};
	class EdSubcat_Personnel_Deserters
	{
		displayName = "Men (Deserters)";
	};
	class EdSubcat_Personnel_Enoch
	{
		displayName = "Men (Livonia)";
	};
	class EdSubcat_Personnel_European
	{
		displayName = "Men (European)";
	};
	class EdSubcat_Personnel_Mediterranean
	{
		displayName = "Men (Mediterranean)";
	};
	class EdSubcat_Personnel_Pacific
	{
		displayName = "Men (Pacific)";
	};
	class EdSubcat_Personnel_Paramilitary
	{
		displayName = "Men (Paramilitary)";
	};
	class EdSubcat_Personnel_SpecialForces
	{
		displayName = "Men (Special Forces)";
	};
	class EdSubcat_Personnel_Story
	{
		displayName = "Men (Story)";
	};
	class EdSubcat_Personnel_Tanoan
	{
		displayName = "Men (Tanoan)";
	};
	class EdSubcat_Personnel_Viper
	{
		displayName = "Men (Viper)";
	};
	class EdSubcat_Personnel_VR
	{
		displayName = "Men (Virtual Reality)";
	};
	class EdSubcat_Pistols
	{
		displayName = "Pistols";
	};
	class EdSubcat_Planes
	{
		displayName = "Planes";
	};
	class EdSubcat_Plants
	{
		displayName = "Plants";
	};
	class EdSubcat_Posters
	{
		displayName = "Posters";
	};
	class EdSubcat_Racing
	{
		displayName = "Racing";
	};
	class EdSubcat_Railways
	{
		displayName = "Railways";
	};
	class EdSubcat_RailwaySegments
	{
		displayName = "Railway Segments";
	};
	class EdSubcat_Religious
	{
		displayName = "Religious";
	};
	class EdSubcat_Residential_City
	{
		displayName = "City";
	};
	class EdSubcat_Residential_Village
	{
		displayName = "Village";
	};
	class EdSubcat_RoadSigns
	{
		displayName = "Road Signs";
	};
	class EdSubcat_Rocks
	{
		displayName = "Rocks";
	};
	class EdSubcat_Seaports
	{
		displayName = "Seaport";
	};
	class EdSubcat_Services
	{
		displayName = "Services";
	};
	class EdSubcat_ShootHouse
	{
		displayName = "Shoot House";
	};
	class EdSubcat_Shops
	{
		displayName = "Shop";
	};
	class EdSubcat_SideSlot
	{
		displayName = "Accessories";
	};
	class EdSubcat_SniperRifles
	{
		displayName = "Sniper & Marksmen Rifles";
	};
	class EdSubcat_Sounds
	{
		displayName = "Sounds";
	};
	class EdSubcat_Sports
	{
		displayName = "Sport & Recreation";
	};
	class EdSubcat_StaticShips
	{
		displayName = "Static Ships";
	};
	class EdSubcat_Storage
	{
		displayName = "Storage";
	};
	class EdSubcat_SubMachineGuns
	{
		displayName = "SMGs";
	};
	class EdSubcat_Submersibles
	{
		displayName = "Submersibles";
	};
	class EdSubcat_Tanks
	{
		displayName = "Tanks";
	};
	class EdSubcat_Targets
	{
		displayName = "Targets";
	};
	class EdSubcat_Terrestrial
	{
		displayName = "Terrestrial";
	};
	class EdSubcat_TEST
	{
		displayName = "TEST";
	};
	class EdSubcat_Tools
	{
		displayName = "Tools";
	};
	class EdSubcat_TopSlot_Collimators
	{
		displayName = "Collimator Sights";
	};
	class EdSubcat_TopSlot_Optics
	{
		displayName = "Optic Sights";
	};
	class EdSubcat_Trains
	{
		displayName = "Trains";
	};
	class EdSubcat_Transportation
	{
		displayName = "Transportation";
	};
	class EdSubcat_Trees
	{
		displayName = "Trees";
	};
	class EdSubcat_Turrets
	{
		displayName = "Turrets";
	};
	class EdSubcat_Uniforms
	{
		displayName = "Uniforms";
	};
	class EdSubcat_Utilities
	{
		displayName = "Utilities";
	};
	class EdSubcat_Vests
	{
		displayName = "Vests";
	};
	class EdSubcat_WarningSigns
	{
		displayName = "Warnings";
	};
};
